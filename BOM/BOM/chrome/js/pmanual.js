...
function update_marco() {
    ...
    marco.innerHTML+="BOM Version:"+bomversion+"<br>";
...
function update_marco_submit() {
	...
	for (var i=0;i<blocks.length;i++) {
			if (blocks[i]) {
				...
				s+="<input type='hidden' value='G"+i+","+dimension(blocks[i])+","+blocks[i].id+","+blocks[i].id+","+countNodes(blocks[i])+"' name='block"+(i+1)+"'>";
			}
	}
	//~ s+=bs;
...
function dale(e) {
	...
	if ( !editing && 
		...
		dialog.innerHTML+="Location: "+getXPath(lastElement)+"<br/>";
