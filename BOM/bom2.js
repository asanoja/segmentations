...
var containerList 	= ["DIV","UL","DL","P","TABLE","TD","SECTION","HEADER","FOOTER","ASIDE","NAV","ARTICLE","OBJECT"];
...
function getWPrima(block,first) {
	...
	if ( (block.type =="PAGE") || (first && block.type != "PAGE")) {
		...
		wprima += openTag("BLOCK",'type="'+block.type+'"');
...
function prepareLogicStructure(go,parent) {
	...
	log=parent;
	if (go.children.length == 1 && (go.tagName.toUpperCase()!="BODY")) {
		log = prepareLogicStructure(go.children[0],parent);
	} else {
		if ( (included(getType(go),["PAGE","CONTAINER","CONTENT_CONTAINER","CONTENT","DEFAULT"])) && (go.getAttribute('bomgeometry')) )
		{
