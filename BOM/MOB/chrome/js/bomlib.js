...
var excludeList 	= ["SCRIPT","STYLE","AREA","HEAD","META","FRAME","FRAMESET","BR","HR","NOSCRIPT","MARQUEE",""];
...
function getRecord(obj,srcurl) {
	...
	var s="";
	if (obj.terminal()) {
		return "BOM,"+sniff()+",none,"+srcurl+","+documentDim().w+","+documentDim().h + ","+ georoot.wordCover() + "," + ac + ","+obj.id+","+obj.dim.x+","+obj.dim.y+","+obj.dim.w+","+obj.dim.h+","+obj.countCover()+","+obj.wordCover()+"\n";
	} else {
...
function traverse(obj) {
	if (!obj) return;
	console.log(obj.id,obj.relativeArea());
	for (var i=0;i<obj.children.length;i++) {
		var child = obj.children[i];
		if (child) {
			traverse(child);
...
function startSegmentation(win,pac,pdc,proclog,psourceurl) {	
		contentWindow = win;
...
function processGeometricStructure(element,parent) {
	...
	if ((dim.w > documentDim().w) || (dim.x > documentDim().w) || ((dim.x+dim.w) > documentDim().w)) {
		console.log("ELEMENT TOO WIDE "+element);
		return;
	}
	if ((dim.h > documentDim().h) || (dim.y > documentDim().h) || ((dim.y+dim.h) > documentDim().h)) {
		console.log("ELEMENT TOO LARGE "+element);
		return;
	}
	if ( (dim.x<0 && dim.w<0) || (dim.y<0 && dim.h<0) ) {
		console.log("ELEMENT OUT OF CONTEXT" + element);
		return;
	}
	if ( (dim.x>documentDim().w) || (dim.y>documentDim().h) ) {
		console.log("ELEMENT OUT OF CONTEXT" + element);
		return;
	}
...
function visible(obj) {
	...
	var xarea=parseFloat((Math.abs(getRect(obj).w))*(Math.abs(getRect(obj).h)));
...
function logicalObject(obj) {
	...
	this.updateBlock = function() {
		var aaa=this.relativeArea().toFixed(2);
		aaa+=" EC: "+this.countCover();
		aaa+=" TC: "+this.wordCover();
...
function geometricObject() {
	...
	this.countCover = function() {
		console.log("cover: ",this.element,$(this.element).find('*').length);
		return($(this.element).find('*').length)
	}
	this.countTextCover = function() {
		var text = $(this.element).text();//this.element.innerText || this.element.textContent;
