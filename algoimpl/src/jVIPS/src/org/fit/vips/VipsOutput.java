...
public static int countWords(String s) {
   if (s.trim()=="") 
       ...
       return(s.replaceAll("[^\\w ]/g", "").split("\\s+").length);
   //return(s.replaceAll("/[^\w ]/g", "" ).split("\s+").length);
}
		...
		else
		{
			...
			if (visualStructure.getNestedBlocks().size() > 0)
			{
				...
				layoutNode.setAttribute("Content", content);
                        ...
                        //tcount = (content.replaceAll("\\s+","").length());
                         
                        tcount = countWords(content);
                        Box curBox = visualStructure.getNestedBlocks().get(0).getBox();
                        //count=((Element) curBox.getNode()).getElementsByTagName("*").getLength();
                        VipsTester.Salida += VipsOutput.part1 + "VB"+visualStructure.getId() + "," + String.valueOf(visualStructure.getX())+","+String.valueOf(visualStructure.getY())+","+String.valueOf(visualStructure.getWidth())+","+String.valueOf(visualStructure.getHeight())+","+count+","+tcount+"\n";
	...
	public void writeXML(VisualStructure visualStructure, Viewport pageViewport)
	{
		try
		{
			...
			Element vipsElement = doc.createElement("VIPSPage");
                        ...
                        org.w3c.dom.Node titlenode = pageViewport.getRootElement().getOwnerDocument().getElementsByTagName("title").item(0);
                        String pageTitle ="";
			if (titlenode != null) {
                            pageTitle = titlenode.getTextContent();
                        }
			...
			vipsElement.setAttribute("order", String.valueOf(pageViewport.getOrder()));
                        ...
                        VipsOutput.tdcount = countWords(pageViewport.getText());
                        VipsOutput.part1 ="JVIPS,cssbox,"+Vips.category+","+Vips.publicUrl+","+String.valueOf(pageViewport.getContentWidth())+","+String.valueOf(pageViewport.getContentHeight())+","+(VipsOutput.tdcount)+","+Vips.publicpDOC+",";
