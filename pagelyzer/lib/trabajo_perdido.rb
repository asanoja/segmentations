...
return (4.0*(r**2.0)*Math.sin(2.0*Math::PI/4.0))/2.0
end

def area(node)
return compute_area(node["elem_width"].to_f, node["elem_height"].to_f)
#~ return node["elem_width"].to_f * node["elem_height"].to_f
end
...
def evalnode(elem,gratio,body,tr,ta,kl,kr)
	...
	return false if !text?(elem) and visible?(elem) and elem.xpath('./*').size>0
	

	newblock = true

	rdocarea = area(elem)/(compute_area(@document.width,@document.height)) #relative_area(elem,body)
	...
	ds = ""
	ds += "Element: '#{elem['id']}' at #{elem.path} (#{elem["elem_left"]} #{elem["elem_top"]} #{elem["elem_width"]} #{elem["elem_height"]}), E.area: #{elem_area}, P.area: #{area(elem.parent)}, Rel.area: #{parent_area}, RelDoc.area: #{rdocarea} #{area(elem)} #{area(body)} #{area(elem)/area(body)}\n" if @verbose
...
def start
	File.open("debug.txt",'w') {|f| f.puts ""}
	begin
	...
	File.open("debug.txt",'a') {|f| f.puts "using #{gratio} is the #{@granularity} of #{lpath} "} if @verbose

	seg = Nokogiri::HTML(File.open(@source_file.gsub(".dhtml",".html")))

	blocks = []
	
	p tr = 1-@granularity #relative thrershold for area of elements
	...
	doc.search("//*[not(child::*)] | //text()").each do |elem|
		...
		while !rooted and !good_partition and !visited?(cur)
			puts "IN #{cur.path} #{cur["id"]} #{cur["class"]}"
			er = evalnode(cur,gratio,body,tr,ta,kl,kr)
			File.open("debug.txt",'a') {|f| f.puts "IN #{cur.path} #{er}"}
			siblings = cur.xpath('../*')
			if er
				...
				if (siblings.size-invalids-text)==0
					partition_quality = 0
				else
					partition_quality = signodes.to_f / (siblings.size-invalids-text)
				end
				...
				if partition_quality>=0.9 or aarea>(1-@granularity)
					...
					if cur.is_a? Nokogiri::HTML::Document
						...
						if ['html'].include?(cur.name)
							...
							partitions.push ([{:elem=>cur,:weight=>partition_quality}] + candidates.collect {|si| {:elem=>si,:weight=>partition_quality}}).flatten
							File.open("debug.txt",'a') {|f| f.puts "--> PARENT"}
							cur = cur.parent
						end
					end
				end
