...
def parse_xml(block,sid)
	...
	src += "<XML>\n"
		src += "<Document url=\"#{escape_html(url.gsub('"',''))}\" algorithm=\"dummy\" Title=\"#{escape_html(title)}\" Version=\"1\" Pos=\"WindowWidth||PageRectLeft:0 WindowHeight||PageRectTop:0 ObjectRectWith:#{width} ObjectRectHeight:#{height}\">\n"
			i = 1
			blocks.each do |b|
				src += parse_xml(b,i)
				i+=1
...
output_file = ARGV[1]	
doc = Document.new
d = doc.loadfile input_file
File.open(output_file,"w") {|f| 
f.puts to_xml(doc.document.url,doc.document.title,doc.document.width,doc.document.height,[d.at("body")])
}
