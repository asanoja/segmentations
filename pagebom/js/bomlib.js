    ...
    for (i = 0; i < log.children.length; i++) {
        if (log.children[i]) {
            if (log.children[i].relativeArea() >= ac) {
                ...
                } else if (log.children[i].isSemantic()) {
                    log.children[i].accept();
                } else {
                    var touch = 0;
                    var stop = false;
            ...
            if ((touch == 0) && (log.children[i].relativeArea() <= ac)) {
                log.children[i].accept();
            }
        }
    }
...
function processLogicStructure(log, level, pid, parent) {
    ...
    if (log.type == "PAGE") {
        ...
        if (log.children.length == 1 && !log.isSemantic()) {
...
//				}
    ...
    this.insertBlock = function() {
        ...
        if (this.visualCuesPresent)
            vc = "VC";
        block.innerHTML = "<span visited='true' class='bomauxtext' style='opacity:1;color:black;font-size:12pt;z-index: 999910000;'>" + this.id + " - " + this.relativeArea().toFixed(4) + "<br>imp:" + this.importance + "</span>";
...
function relativeAreaElement(element) {
	if (page) {
		var cont = 0;
		for (var i = 0; i < 100; i += 10) {
			if ((100 * this.dim.w / documentDim().w) > i)
				cont++;
		}
		var dw = cont;
		cont = 0;
		for (var i = 0; i < 100; i += 10) {
			if ((100 * this.dim.h / documentDim().h) > i)
				cont++;
		}
		var dh = cont;
